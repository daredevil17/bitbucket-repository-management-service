...
func main() {
	...
	// Call Create
	log.Printf("calling Create function...")
	resp, err := http.Post(*address+"/todo", "application/json", strings.NewReader(fmt.Sprintf(`
		{
			"api":"v1",
			"toDo": {
				"title":"title (%s)",
	...
	log.Printf("calling READ function for %s", created.ID)
	// Call Read
	resp, err = http.Get(fmt.Sprintf("%s%s/%s", *address, "/todo", created.ID))
	if err != nil {
	...
	req, err := http.NewRequest("PUT", fmt.Sprintf("%s%s/%s", *address, "/todo", created.ID),
	...
	// Call ReadAll
	resp, err = http.Get(*address + "/todo/all")
	if err != nil {
	...
	// Call Delete
	log.Printf("calling delete function...")
	req, err = http.NewRequest("DELETE", fmt.Sprintf("%s%s/%s", *address, "/todo", created.ID), nil)
	resp, err = http.DefaultClient.Do(req)
	if err != nil {
